        ...
        <tbody>
            <tr v-for="(item, Nro) in filterData" v-bind:key="item.id">
              <!--  -->
                ...
                <td data-title="FECHA">{{ item.fecha_atencion}}</td>
...
import DiagnosticAttention from '../components/Patient/HistoryPatient.vue';
...
const showAttention = async () => {
  try {
    result.value = await patientService.showHistoryPatient()

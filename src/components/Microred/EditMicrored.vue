<template>
  <div class="container-form-modal">
    <form action="" v-on:submit.prevent="editMicrored" class="content-form-modal">
      <fieldset class="fieldset-form-modal">
        <legend class="legend-form-modal">
          <span class="title-form-modal">EDITAR MICRORED</span>
        </legend>
        <section class="register-form-modal">
          <h3>CODIGO DE MICRORED: {{ props.codigo }}</h3>
...
const editMicrored = async() =>{
  ...
  try {
    ...
    if (resultSwal.isConfirmed) {
      result.value = await microredService.updateMicrored(props.codigo, microred);
      if(result.value.ok){
        Swal.fire({
